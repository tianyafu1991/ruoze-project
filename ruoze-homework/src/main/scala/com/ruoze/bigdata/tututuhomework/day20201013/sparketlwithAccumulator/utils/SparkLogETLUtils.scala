...
object SparkLogETLUtils extends Serializable {
  ...
  /*val m:(DbSearcher) => Method =getMethod _

  val s:(()=>DbConfig,String) => DbSearcher = getDbSearcher _

  def c:()=>DbConfig = getDbConfig _

  val l:(String,DbSearcher,Method)=>Access = parseLog _*/


  def main(args: Array[String]): Unit = {
    /*val dbPath = "ip2region.db"
    val log : String = ""
    val method = m(s(c, dbPath))
    val access = l(log, s(c, dbPath), m(s(c, dbPath)))*/
  }
